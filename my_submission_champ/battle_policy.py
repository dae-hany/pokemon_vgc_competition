...
Phase 1~3 ported from my_submission_battle:
- Top-K action candidates, damage cache, greedy cache per node
- KO-priority rollout, Trick Room in evaluate_state, higher boost weight
- Focus-fire coordination (JJJ-inspired): both slots target same enemy
- No expand_explore_prob: deterministic structured expansion
"""
import random
import time
import math
...
class EnhancedBattlePolicy(BattlePolicy):
    ...
    DETERMINIZATION_SCENARIOS = 5
    SOFT_TIME_FRACTION = 0.85

    def __init__(self, time_limit_ms: int = 5000):
    ...
    def _build_root_determinization_scenarios(self, state: State) -> List[dict]:
        scenarios = []
        enemy_actives = list(state.sides[1].team.active)
        for s in range(self.DETERMINIZATION_SCENARIOS):
            ...
            for enemy in enemy_actives:
                ...
                scenario["append"].append(rng.sample(candidates, sample_n))
    ...
    def _apply_determinization_to_state_inplace(self, state: State, scenario: dict):
        ...
        for i, enemy in enumerate(enemy_actives):
            ...
            if not app[i]:
                continue
            room = 4 - len(enemy.battling_moves)
            if room <= 0:
                continue
            enemy.battling_moves += [BattlingMove(m) for m in app[i][:room]]
    ...
    # --- Rollout (Phase 2: KO-priority per slot) ---
...
class ChampionshipWeatherBattlePolicy(BattlePolicy):
    """
    MCTS를 항상 먼저 실행하여 기본 행동 결정.
    유리한 날씨(RAIN/SUN) 세터가 한 마리라도 있으면 해당 슬롯만 날씨 기술로 교체.
    """

    def __init__(self, inner: Optional[BattlePolicy] = None, time_limit_ms: int = 5000):
        self.params = BattleRuleParam()
        self.inner = inner if inner is not None else EnhancedBattlePolicy(time_limit_ms=time_limit_ms)
        self.force_threshold = 80.0
    ...
    def decision(self, state: State, opp_view: Optional[TeamView] = None) -> list[BattleCommand]:
        ...
        try:
            ...
            pass
        try:
            inner_actions = self.inner.decision(state, opp_view)
        except Exception:
            gp = GreedyBattlePolicy()
            gp.set_params(self.params)
            inner_actions = gp.decision(state)
        try:
            forced = self._maybe_force_weather(state, inner_actions)
